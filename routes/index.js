...
//End point for changing contract owner from an api
// For learning purposes we will only accept numbers from 0-9 and use it as an index for ganache accounts
router.get("/changeOwner", async function (req, res, next) {
  // New contract owner is :
  let addIndex = req.query.address;

  if (isNaN(addIndex) || addIndex > 9 || addIndex < 0) {
    res.send({
      error: "Value is not a valid number. Please choose a number from 0-9",
    });
  } else {
    // making a Web3 object
    const Web3 = require("web3");
    ...
    // A second address who you want to change the owner too
    const newAddress = accounts[addIndex];
    // Grabing Test contract from the network by ABI and address
    let TestContract = new web3.eth.Contract(ABI, contractAddress);
    // ABI encoding the function call
    const _data = TestContract.methods.setOwner(newAddress).encodeABI();
